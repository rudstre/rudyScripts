function seqTrend = seqDetectOverview(PPSeq, seqs, behavStates, amp_thresh, f_len)
    ...
    for seq = seqs
        ...
        [seqTrend{seq}, ts] = histcounts(PPSeq.events.ts(PPSeq.events.type == seq & PPSeq.events.event_amp > amp_thresh), 'BinEdges',0:86400);
        
        % Smooth the histogram using a moving sum
        sm = movsum(seqTrend{seq}, f_len, 'Endpoints', 'fill');
        
        % Plot the smoothed sequence detections per hour
        plot(ts(1:end-1) / 60 / 60, sm / f_len, 'LineWidth', 2);
