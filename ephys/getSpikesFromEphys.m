function [spikeWaveforms, thresholds] = getSpikesFromEphys(signalData, impedances, thresholdMultiplier, detectionDirection, timeWindow)
...
% Calculate the threshold for spike detection using the median absolute deviation
thresholds = mad(signalData, [], 1) * thresholdMultiplier + median(signalData,1);
...
for channelIdx = 1:size(signalData, 2)
    ...
    % Detect peaks in the data that exceed the threshold
    [~, peakLocations] = findpeaks(channelData * signMultiplier, ...
        'MinPeakHeight', thresholds(channelIdx), 'MinPeakDistance', 300);
