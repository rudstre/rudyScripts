...
while true
    switch charPress
        ...
        case double('j')
            ...
            clusters = joinClusters(clusters, clustersToJoin);
        case double('s')
            [x, ~] = ginput(1);
...
%% User selection of cluster corresponding to three stages of sleep (NREM1/2, NREM3, REM)
...
%% Nested Functions
    ...
    function clusters = joinClusters(clusters,clustersToJoin)
    indicesToJoin = clusters == max(clustersToJoin);
    clusters(indicesToJoin) = min(clustersToJoin);

    indicesToMoveOver = clusters > max(clustersToJoin);
    clusters(indicesToMoveOver) = clusters(indicesToMoveOver) + 1;
    end
